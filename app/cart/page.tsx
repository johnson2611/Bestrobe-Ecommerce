  ...
  if (items.length === 0) {
    return (
      <main className="mx-auto max-w-2xl px-6 py-24 text-center">
        ...
        <p className="mt-3 text-[var(--color-ink)]/60">Add something from the collection.</p>
        <Link
          href="/shop"
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-16">
      ...
      <div className="mt-8 divide-y divide-[var(--color-line)] border-y border-[var(--color-line)]">
        {items.map((item) => (
          <div key={`${item.productId}-${item.size}`} className="flex flex-col gap-4 py-6 sm:flex-row sm:items-center">
            <div className="h-20 w-16 flex-shrink-0 rounded-lg" style={{ background: item.color }} />

            <div className="flex-1">
              <div className="flex items-start justify-between">
                <div>
                  <p className="font-medium text-[var(--color-ink)]">{item.name}</p>
                  <p className="text-sm text-[var(--color-ink)]/60">Size {item.size} · {formatNaira(item.price)}</p>
                </div>
                <button
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="font-display text-2xl text-[var(--color-ink)]">{formatNaira(subtotal)}</p>
        </div>
      </div>
